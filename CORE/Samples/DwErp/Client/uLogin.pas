...
uses
  nfeNFDXML, JclWin32, JclSysInfo, ShlObj, ComObj, ActiveX, Registry,
  JvJVCLUtils, JvTypes,
  Windows, Messages, SysUtils, Variants, Classes, Graphics, Controls,
  Forms, Dialogs, ExtCtrls, StdCtrls, JvExStdCtrls, JvButton, JvCtrls, DB,
  DBClient, JvExControls, JvLabel, dxGDIPlusClasses, JvComponentBase,
  JvFormTransparent, jpeg, Mask, JvPageList, ushldlg, JvComputerInfoEx,
  AdvCombo, ColCombo, IdBaseComponent, IdComponent, IdTCPConnection,
  IdTCPClient, IdHTTP, Vcl.ComCtrls, dbxJSON ,
JvMemoryDataset, Vcl.Imaging.pngimage
  ;
...
type
  TFrmLogin = class(TForm)
    ...
    procedure BtnAdminConfigClick(Sender: TObject);
...
uses  uPrincipal, uEnterSenha , uAdminConfig,
  uSplashConexao, uDM,  Ufuncoes;
{$R *.dfm}

procedure TFrmLogin.BtnAdminConfigClick(Sender: TObject);
begin
  FrmEnterSenha := TFrmEnterSenha.Create(nil);
  try
    if FrmEnterSenha.ShowModal = mrCancel then
      Exit;

    if not AnsiSameText(FrmEnterSenha.edSenha.Text, 'PONTOSL' + FormatDateTime('dd', Now)) then
    begin
      Application.MessageBox('Senha incorreta.' + #13#10#13#10 + 'Digite novamente.', PChar(Application.Title), MB_OK + MB_ICONWARNING);
      Exit;
    end;

    FrmAdminConfig := TFrmAdminConfig.Create(nil);
    try
      FrmAdminConfig.ShowModal;
    finally
      FrmAdminConfig.Release;
    end;

  finally
    FrmEnterSenha.Release;
  end;
end;
...
begin
  ...
  begin
    ...
    if Assigned(FrmPrincipal.DXFunc) then
      FreeAndNil(FrmPrincipal.DXFunc);
       ...
       //   if not Funcoes.DentroFirma then
        //     CreateShortcut(ExtractFilePath(Application.ExeName) + 'SuporteCliente.exe', '', ExtractFilePath(Application.ExeName), 'Suporte OnLine - Point Informática', '');
          ...
          try
            ...
            sSQL := 'select IDSYS_POINT_CLIENTE, STATUS, RAZAO_SOCIAL, CNPJ, MENSAGEM from SYS_POINT_CLIENTE where CNPJ = ' + QuotedStr(edCNPJ.Text);
    ...
    1:
      begin // Login de Usuário
        ...
        try
          ...
          dm.frxMailExport1.ConfurmReading := True;
          dm.frxMailExport1.SmtpPort := 587;
          dm.frxMailExport1.SmtpHost := dm.CdsEmpresa.FieldByName('SMTPEMAIL').AsString;
          dm.frxMailExport1.Password := dm.CdsEmpresa.FieldByName('SENHAEMAIL').AsString;
          dm.frxMailExport1.Login := dm.CdsEmpresa.FieldByName('USUARIOEMAIL').AsString;
          dm.frxMailExport1.UseIniFile := False;
          dm.frxMailExport1.ShowDialog := True;
          dm.frxMailExport1.ShowExportDialog := True;
          dm.frxMailExport1.FromName := Funcoes.CurrentUser.UserLogin;
          dm.frxMailExport1.FromCompany := dm.CdsEmpresa.FieldByName('RAZAO_SOCIAL').AsString;
          dm.frxMailExport1.FromMail := dm.CdsEmpresa.FieldByName('email').AsString;
          dm.frxMailExport1.Lines.Clear;
          dm.frxMailExport1.Signature.Clear;
          dm.frxMailExport1.ExportFilter := dm.frxPDFExport1;
...
procedure TFrmLogin.ColumnComboBox1Change(Sender: TObject);
begin
  edCNPJ.Text := ColumnComboBox1.ColumnItems[ColumnComboBox1.ItemIndex, 0];
end;
...
begin
  ...
  ColumnComboBox1.Visible := Funcoes.MaquinaDesenvolvedor;
  if Funcoes.MaquinaDesenvolvedor then
  begin
      ...
      begin
        ...
        begin
          ...
          begin
            ...
            begin
              Strings.Add(Trim(aTempNode2.AttributeByName['cnpj']));
              Strings.Add(Trim(aTempNode2.AttributeByName['descricao']));

              Strings.Add(Trim(aTempNode2.AttributeByName['file_path']));
            end;
          end;
        end;
