...
type
  ...
  public
    ...
    function InSertUpdate(Pdataset: TFDMemTable; Ptabela: string): string;
...
function TClassePonto.InSertUpdate(Pdataset: TFDMemTable; Ptabela: string): string;
var
  i: integer;
  FfieldName,Ffield ,Fmatching: string;

begin
  result := ' update or insert into  ' + Ptabela + ' ( ';
  FfieldName := EmptyStr;
  Ffield := EmptyStr;
  Fmatching := 'matching (';
  Pdataset.First;
      ...
      begin
        ...
        case Pdataset.Fields.Fields[i].DataType of
          ...
          ftCurrency, ftBCD, ftFloat:
            Ffield := Ffield + FormatCurr('0.00', StrToCurr(Pdataset.Fields.Fields[i].FieldName)) + ', ';
  ...
  result := result + copy( FfieldName , 1 , ( length(FfieldName)-2 ) )  +
  ' ) values ( ' + copy( Ffield , 1 , ( length(Ffield )-2 ) ) +
  ' ) ' + copy( Fmatching , 1 , ( length(Fmatching )-2 ) )  + ' )';
