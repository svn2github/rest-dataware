...
begin
  ...
  ShowMessage
    ('Desenvolvido a partir da necessidade PESSOAL de converter ZEOS para RestDW '
    + 'após algumas tentativas frustradas de usar o "reFind".' + sLineBreak +
    sLineBreak +
    'Facilmente poderá ser adaptado a quaquer necessidade como opção ao GReplace, por exemplo, '
    + sLineBreak +
    'com a vantagem de não só substituir mas ADICIONAR linhas de código tanto ao .DFM quanto ao .PAS'
    + sLineBreak + sLineBreak + 'Qualquer dúvida: ' + sLineBreak + sLineBreak +
    'skype: mikromundo.com ou ' + sLineBreak + 'email: mikromundo@gail.com' +
    sLineBreak + sLineBreak + 'Flávio Motta( www.mikromundo.com )');
...
var
  ...
  _frameBarraSup, _rzButton, _panel, _rzlistbox,

    _StoreProc, _StoreProcLin, _tot, Alterou, NumLinhas, f, r, contadorTabOrder,
    contadorRzButton: Integer;

  SearchRec: TSearchRec;

  DfmFilename, Filename, linha, linha2, linha3, linhaSQL, linhaDataSource,
    linhaLeft, linhaTop, NewFileName: string;
  ...
  _pular,
    ...
    _itens_descartar,
  // dependendo da Engine de origem. Para evitar conflito de propriedades com nomes em comum entre Engines

  _itens_composto, _itens_uses, _itens_alt_dfm, _itens_add_dfm, _itens_pas,
    _itens_filtros, _itens_add_dfm_PropRestDataBase,
  ...
  aEngineOrigem, aEngineOrigemComposto, aRestDW, aNovos, aUses,
    aCode_EngineOrigem, aCode_RestDW, aAddPropRestDataBase
    : array [0 .. 50] of String;

  aFiltroArqs: array [1 .. 2] of String;

  aArquivos: array [1 .. 5000] of String;

  strArqOrigem, strArqDestino, strArqTemp: TStringList;
...
begin
  ...
  *)
  ...
  begin
    ...
    Inc(_itens_alt_dfm);
    aEngineOrigem[_itens_alt_dfm] := ' Connection =';
    // altere pra suas definicoes de conexao da ZEOS
  ...
  else if cbxEngine.Text = 'IBX' then
  begin
  ...
  Inc(_itens_add_dfm_PropRestDataBase);
  aAddPropRestDataBase[_itens_add_dfm_PropRestDataBase] :=
    'Login = ''testserver'''; // altere para seu login
  Inc(_itens_add_dfm_PropRestDataBase);
  aAddPropRestDataBase[_itens_add_dfm_PropRestDataBase] :=
    'Password = ''testserver'''; // altere para seu usuario
  Inc(_itens_add_dfm_PropRestDataBase);
  ...
  aCode_RestDW[_itens_pas] :=
    '.ExecSql( _Erro_DW ) then ShowMessage( _Erro_DW );';
  // acrescentar if not antes
  Inc(_itens_pas);
  aCode_EngineOrigem[_itens_pas] := 'ExecSql;'; // Quaquer query...
  aCode_RestDW[_itens_pas] :=
    'if not ExecSql( _Erro_DW ) then ShowMessage( _Erro_DW );';
  // Mizael
  if cbxEngine.Text = 'Zeos' then
  begin
    Inc(_itens_pas);
  ...
  if (cbxEngine.Text <> 'Zeos') and (cbxEngine.Text <> 'IBX') then
  begin

    ShowMessage
      ('Ajude o projeto. A conversão é feita apenas de ZEOS para RestDW por enquanto.');
    Exit
  ...
  begin
    ...
    ShowMessage
      ('A pasta de arqs. a serem convertidos( ORIGINAL ) foi criada em : ' +
      sLineBreak + edPastaOriginal.Text + sLineBreak + sLineBreak +
  ...
  begin
    ...
    ShowMessage('A pasta de arqs. migrados( CONVERTIDOS ) foi criada em : ' +
      sLineBreak + edPastaConvertido.Text + sLineBreak + sLineBreak +
  ...
  begin
    ...
    if FindFirst(edPastaOriginal.Text + aFiltroArqs[a], faAnyFile, SearchRec) = 0
    then
    begin
      Inc(_tot);
  ...
  begin
    ...
    labTotArqs.Caption := 'Processado(s) ' + pg.Position.ToString + ' de ' +
      _tot.ToString;
        ...
        begin
          ...
          begin
            ...
            try
              ...
              if (Pos(Ansilowercase(aEngineOrigem[I]), Ansilowercase(linha)) > 0)
              then
              begin
                ...
                // Mizael
                { if I <= 3 then
                  linha := StringReplace(linha, aEngineOrigem[I], aRestDW[I],
          ...
          //
          if (Pos('database =', Ansilowercase(linha)) > 0) then
          // Prop. DATABASE serve tanto pra ZEOS quanto pra IBX
          begin
            ...
            repeat
              ...
              if sToken1 = '<' then
                sToken2 := 'end>'
              else if sToken1 = '(' then
                sToken2 := ''')';

              for I := 1 to _itens_composto do
              begin

                if (Pos(Ansilowercase(aEngineOrigemComposto[I]),
                  Ansilowercase(linha)) > 0) and (Pos('<>', linha) = 0) then
                begin
    ...
    begin
      ...
      strArqDestino.Text := StringReplace(memo_Original.lines.Text,
        aEngineOrigem[1], aRestDW[1], [rfReplaceAll, rfIgnoreCase]);
        ...
        begin
          ...
          begin
            ...
            try
              ...
              if (Pos(Ansilowercase(aUses[I]), Ansilowercase(strArqDestino.Text)
                ) = 0) then
              begin
      ...
      begin
        ...
        begin
          ...
          try
            ...
            if (Pos(Ansilowercase(aCode_EngineOrigem[I]), Ansilowercase(linha)
              ) > 0) then
            begin
              ...
              begin
                ...
                linha := StringReplace(linha, aCode_EngineOrigem[I],
                  aCode_RestDW[I], [rfReplaceAll, rfIgnoreCase]);
